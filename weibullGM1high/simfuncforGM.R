##XL revised  *April30. R to this *Dec10.R
## Added "gamma.true" in simID.LT.noseed(), which is need in simData file;
...
simID.LT.noseed <- function(mu1,mu2,sigma1,sigma2,p1,p2,x1, x2, x3, beta1.true, beta2.true, beta3.true,
                            ...
                            kappa1.true, kappa2.true, kappa3.true, theta.true, SigmaV.true=NULL, cens, lt.type = "unif", lt,risks.status,frailty_distribution)
{
  ...
  #frail;ty parameter could be from gamma or log-normal. We have both scenarios below:
  
  #For log-Normal Distribution where w~exp{N(0,sigma^2)}, we have three scenarios:
  #sigma^2=0.25 or 1
  #gamma= 0.25 or 1
  ...
  if (frailty_distribution == "GaussianMixture") {
    # Step 1: Sample component labels
    component <- sample(c(1, 2), size = n, replace = TRUE, prob = c(p1, p2))
    
    # Step 2: Generate data on the log scale
    gamma_log <- ifelse(component == 1,
                        rnorm(n, mean = mu1, sd = sigma1),
                        rnorm(n, mean = mu2, sd = sigma2))
    
    # Step 3: Transform to the positive scale
    gamma.true <- exp(gamma_log)
  }
  ...
  ret <- data.frame(cbind(y1, delta1, y2, delta2, L, gamma.true)) #added gamma.true
  return(ret)
}
...
#q_n=10
genWB.simData.noseed.allcontin<-  function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  ...
  Y.tmp = data.frame(simID.LT.noseed(mu1,mu2,sigma1,sigma2,p1,p2,x1, x2, x3, beta1.true, beta2.true, beta3.true,
                                     alpha1.true, alpha2.true, alpha3.true,
  ...
  # for (i in 6:15){          #5 survival outcomes + 10 covariates, without gamma.true;
  #   Y[,i]=scale(Y[,i])   
  # }
  
  for (i in 7:16){          #6 survival outcomes + 10 covariates, with gamma.true;
    Y[,i]=scale(Y[,i])   #I added gamma.true in the function simID.LT.noseed()
  }                               #scale() to standandize. 
  
  data = data.frame(Y)
  
  delta1 = Y$delta1; delta2 = Y$delta2
  y1 = Y[,1]; delta1 = Y[,2]; y2 = Y[,3]; delta2 = Y[,4]; l=Y[,5]; gamma.true=Y[,6]
...
genWB.simData.noseed.allcontin.20cov<- function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
...
genWB.simData.noseed.allcontin.12cov<- function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
  ...
  Y.tmp = data.frame(simID.LT.noseed(mu1,mu2,sigma1,sigma2,p1,p2,x1, x2, x3, beta1.true, beta2.true, beta3.true,
                                     alpha1.true, alpha2.true, alpha3.true,
                                     kappa1.true, kappa2.true, kappa3.true, theta.true, SigmaV.true=NULL, 
                                     cens,lt.type="unif", lt,risks.status,frailty_distribution),data0)
  
  
  ## Make sure left-truncated data
  Y = Y.tmp %>% filter(y1 > L)
  
  # for (i in 6:17){          #5 survival outcomes + 12 covariates, without gamma.true;
  #   Y[,i]=scale(Y[,i])   
  # }
  
  for (i in 7:18){          #6 survival outcomes + 12 covariates, with gamma.true;
    Y[,i]=scale(Y[,i])   #I added gamma.true in the function simID.LT.noseed()
  }                               #scale() to standandize. 
  ...
  return(list(Y = Y,
              lin.pred = lin.pred,
              lin.pred.oracle=lin.pred.oracle,
              data = data,       #now data contains gamma.true
              frailty = frailty, frailty,       #why there are two frailty terms? 
              WBparamNames = WBparamNames,
              WBpara = WBpara)) 
}  

genWB.simData.noseed.allcontin.14cov<- function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
  ...
  # for (i in 6:19){          #5 survival outcomes + 14 covariates, without gamma.true;
  #   Y[,i]=scale(Y[,i])   
  # }
  
  for (i in 7:20){          #6 survival outcomes + 14 covariates, with gamma.true;
    Y[,i]=scale(Y[,i])   #I added gamma.true in the function simID.LT.noseed()
  }                               #scale() to standandize. 
...
genWB.simData.noseed.allcontin.15cov<- function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
  ...
  # for (i in 6:20){          #5 survival outcomes + 15 covariates, without gamma.true;
  #   Y[,i]=scale(Y[,i])   
  # }
  
  for (i in 7:21){          #6 survival outcomes + 15 covariates, with gamma.true;
    Y[,i]=scale(Y[,i])   #I added gamma.true in the function simID.LT.noseed()
  }                               #scale() to standandize. 
...
genWB.simData.noseed.allcontin.18cov<- function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
  ...
  # for (i in 6:23){          #5 survival outcomes + 18 covariates, without gamma.true;
  #   Y[,i]=scale(Y[,i])   
  # }
  
  for (i in 7:24){          #6 survival outcomes + 18 covariates, with gamma.true;
    Y[,i]=scale(Y[,i])   #I added gamma.true in the function simID.LT.noseed()
  }                               #scale() to standandize. 
...
genWB.simData.noseed.allcontin.16cov<- function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
  ...
  # for (i in 6:21){          #5 survival outcomes + 16 covariates, without gamma.true;
  #   Y[,i]=scale(Y[,i])   
  # }
  
  for (i in 7:22){          #6 survival outcomes + 16 covariates, with gamma.true;
    Y[,i]=scale(Y[,i])   #I added gamma.true in the function simID.LT.noseed()
  }                               #scale() to standandize. 
...
genWB.simData.noseed.allcontin.19cov<- function(mu1,mu2,sigma1,sigma2,p1,p2,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n , rho,frailty,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
  ...
  # for (i in 6:24){          #5 survival outcomes + 19 covariates, without gamma.true;
  #   Y[,i]=scale(Y[,i])   
  # }
  
  for (i in 7:25){          #6 survival outcomes + 19 covariates, with gamma.true;
    Y[,i]=scale(Y[,i])   #I added gamma.true in the function simID.LT.noseed()
  }                               #scale() to standandize. 
...
genWB.simData.noseed.allcontin.grp<- function(G1=G1,G2=G2,G3=G3,G4=G4,G1.distr=G1.distr,G2.distr=G2.distr,G3.distr=G3.distr,G4.distr=G4.distr,mu.bernoulli=mu.bernoulli,weibull.param.log,c1,c2,lt1,lt2,beta1.true,beta2.true,beta3.true,n = n, rho=rho,frailty = T,risks.status,theta.given,frailty_distribution){
  #frailty parameter:
  theta.true = ifelse(frailty == T, theta.given, 0)
  #Weibull parameters:
  # log.kappa1.true = -9.98
...
logLike.weibull.SCR.SM.LT.beta.fixed <- function(para,beta1.from.varsel,beta2.from.varsel,beta3.from.varsel, y1, y2, delta1, delta2, l, Xmat1=NULL, Xmat2=NULL, Xmat3=NULL, frailty=TRUE)
{
  ##
  kappa1    <- exp(para[1])
  alpha1 <- exp(para[2])
...
FreqID.LT <- function(theta.init,Y, lin.pred, data, model = "semi-Markov", startVals, frailty=TRUE, method)
{	
  ...
  if(model == "semi-Markov")
  {
    if (method == "optim"){
      ...
      value <- list(startValues=startvalues,estimate=fit1$par, H=fit1$hessian, logLike=-fit1$value, code=fit1$convergence)#, Xmat=list(Xmat1, Xmat2, Xmat3))
    }
    if (method == "nlm"){
      cat("Fitting illness-death model with Weibull baseline hazards ... this should take < 1 min \n")
      ...
      value = list(startValues=startvalues,estimate=fit1$est, H=fit1$hessian, logLike=-fit1$minimum, code=fit1$code)
    }
  }
  ##
  # if(model == "semi-Markov")
...
FreqID.LT.oracle.overlapping <- function(theta.init,Y, lin.pred.oracle, data.oracle.1,data.oracle.2,data.oracle.3, model = "semi-Markov", startVals, frailty, method)
{	
